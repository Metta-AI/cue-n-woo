...
The policy does not pass temperature or other sampling settings to Claude. Game-side generation settings such as the judge token cap, local worker temperature, FLAS strength, and concept selection are controlled by the Coworld game config and are shown to the policy only through the observation when appropriate.
...
The private transcript section is rendered separately from the JSON to make prior judge responses hard to miss:
...
For a player, `me` includes that player's private judge transcript, submitted proposals, and submitted answers. Player-indexed fields (`public_questions`, `counts`) are lists ordered by slot:
